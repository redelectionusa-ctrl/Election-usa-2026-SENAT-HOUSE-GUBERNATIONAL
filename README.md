...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .candidate-card.winner { border-color:rgba(255,215,150,0.4); box-shadow:0 0 18px rgba(255,215,150,0.1); }
    ...
    .cand-photo.winner-photo { border-color:rgba(255,215,150,0.8); box-shadow:0 0 12px rgba(255,215,150,0.3); }
    .cand-name { font-size:1rem; font-weight:700; color:white; margin-bottom:0.2rem; }
    .cand-desc { font-size:0.7rem; color:rgba(255,255,255,0.5); margin-bottom:0.5rem; line-height:1.3; }
    .cand-score { font-size:1.6rem; font-weight:800; margin-bottom:0.2rem; } .cand-score.r { color:#ff9e8f; } .cand-score.d { color:#8fc1ff; }
    ...
    .chart-tooltip { position:absolute; background:rgba(20,20,30,0.95); color:white; padding:7px 10px; border:1px solid rgba(255,255,255,0.25); font-size:11px; font-weight:600; pointer-events:none; box-shadow:0 5px 18px rgba(0,0,0,0.6); z-index:100; text-align:center; line-height:1.3; white-space:nowrap; }
...
<div class="main-content">
  ...
  <div class="cat-panel active" id="cat-house">
    <div class="race-list" id="house-races">
      <div class="race-item active" data-race="house-wa3">
        <div class="race-info"><h3>Washington · 3e District</h3><p>John Braun (R) vs Marie Gluesenkamp Perez (D) *</p></div>
        <div class="race-rating-badge badge-tossup">Tossup</div>
      </div>
      <div class="race-item" data-race="house-ca22">
        <div class="race-info"><h3>Californie · 22e District</h3><p>David Valadao (R) * vs Jasmeet Bains (D)</p></div>
        <div class="race-rating-badge badge-lean-d">Lean D</div>
      </div>
    </div>
  </div>
  ...
  (function() {
    const racesData = {
      'senate-tx': {
        ...
        marginError:'±3.2%', sample:'2 814 LV', pollDates:'12-15 Sep 2026',
        polls:[
          { date:'Jan', rep:47, dem:44 },{ date:'Fév', rep:44, dem:43 },{ date:'Mar', rep:44, dem:43 },
          { date:'Avr', rep:45, dem:44 },{ date:'Mai', rep:47, dem:42 },{ date:'Jun', rep:46, dem:44 },
          { date:'Jul', rep:44, dem:44 },{ date:'Aoû', rep:47, dem:45 },{ date:'Sep', rep:48, dem:46 }
        ],
        primaryDate:'4 mars 2026',
        primaries:{
          rep:{ title:'Primaire Républicaine', note:'Second tour le 26 mai 2026 entre Cornyn et Paxton.',
            candidates:[
              { name:'Ken Paxton', initials:'KP', desc:'Procureur général · Soutenu par Trump', score:'48.8%', class:'r', badge:'badge-runoff', badgeText:'Second tour', isWinner:true },
              { name:'John Cornyn', initials:'JC', desc:'Sénateur sortant', score:'41.3%', class:'r', badge:'badge-runoff', badgeText:'Second tour', isWinner:true },
              { name:'Wesley Hunt', initials:'WH', desc:'Représentant (TX-38)', score:'~10%', class:'r', badge:'badge-eliminated', badgeText:'Éliminé', isWinner:false }
            ],
            polls:[
          ...
          dem:{ title:'Primaire Démocrate', note:'James Talarico a remporté l\'investiture avec 53.1%.',
            candidates:[
              { name:'James Talarico', initials:'JT', desc:'Représentant d\'État', score:'53.1%', class:'d', badge:'badge-winner', badgeText:'🏆 Vainqueur', isWinner:true },
              { name:'Jasmine Crockett', initials:'JC', desc:'Représentante (TX-30)', score:'45.6%', class:'d', badge:'badge-eliminated', badgeText:'Battue', isWinner:false },
              { name:'Ahmad Hassan', initials:'AH', desc:'Avocat', score:'1.3%', class:'d', badge:'badge-eliminated', badgeText:'Battu', isWinner:false }
            ],
            polls:[
      ...
      'house-wa3': {
        ...
        candidates: [
          { name:'John Braun', party:'R', initials:'JB', percent:47, desc:'Ancien chef de la minorité au Sénat d\'État' },
          { name:'Marie Gluesenkamp Perez', party:'D', initials:'MP', percent:48, desc:'Représentante sortante · Élue en 2022' }
        ],
        projections:{ rSafe:10, rLikely:15, rLean:15, tossup:30, dLean:18, dLikely:12 },
        seatsInfo:'Sièges projetés : 218 R - 217 D · Fourchette : +5D à +15R',
        marginError:'±3.5%', sample:'1 100 LV', pollDates:'12-15 Sep 2026',
        polls:[
          { date:'Jan', rep:45, dem:47 },{ date:'Fév', rep:44, dem:48 },{ date:'Mar', rep:46, dem:46 },
          { date:'Avr', rep:45, dem:49 },{ date:'Mai', rep:47, dem:46 },{ date:'Jun', rep:46, dem:47 },
          { date:'Jul', rep:44, dem:48 },{ date:'Aoû', rep:46, dem:47 },{ date:'Sep', rep:47, dem:48 }
        ],
        primaryDate:'5 août 2025',
        primaries:{
          rep:{ title:'Primaire Républicaine (Top 2)', note:'Washington utilise un système de primaire unique. John Braun est arrivé en tête avec 55%.',
            candidates:[
              { name:'John Braun', initials:'JB', desc:'Ancien chef de la minorité au Sénat d\'État', score:'55%', class:'r', badge:'badge-qualified', badgeText:'Qualifié', isWinner:true },
              { name:'Joe Kent', initials:'JK', desc:'Ancien militaire · Candidat en 2022', score:'45%', class:'r', badge:'badge-eliminated', badgeText:'Battu', isWinner:false }
            ],
            polls:[
              { institute:'Résultat primaire', date:'5 Aoû 2025', values:['55%','45%'] },
              { institute:'SurveyUSA', date:'Juil 2025', values:['52%','48%'] }
            ]
          },
          dem:{ title:'Primaire Démocrate (Top 2)', note:'Perez s\'est qualifiée avec 46%. Elle est la seule démocrate.',
            candidates:[
              { name:'Marie Gluesenkamp Perez', initials:'MP', desc:'Représentante sortante', score:'46%', class:'d', badge:'badge-qualified', badgeText:'Qualifiée', isWinner:true }
            ],
            polls:[
      ...
      'house-ca22': {
        ...
        candidates: [
          { name:'David Valadao', party:'R', initials:'DV', percent:46, desc:'Représentant sortant · Élu depuis 2020' },
          { name:'Jasmeet Bains', party:'D', initials:'JB', desc:'Médecin · Députée d\'État de Californie', percent:50 }
        ],
        projections:{ rSafe:5, rLikely:10, rLean:12, tossup:25, dLean:28, dLikely:20 },
        seatsInfo:'Sièges projetés : 218 R - 217 D · Fourchette : +3D à +12R',
        marginError:'±3.8%', sample:'950 LV', pollDates:'12-15 Sep 2026',
        polls:[
          { date:'Jan', rep:45, dem:49 },{ date:'Fév', rep:44, dem:50 },{ date:'Mar', rep:46, dem:48 },
          { date:'Avr', rep:43, dem:51 },{ date:'Mai', rep:45, dem:49 },{ date:'Jun', rep:44, dem:50 },
          { date:'Jul', rep:46, dem:49 },{ date:'Aoû', rep:45, dem:50 },{ date:'Sep', rep:46, dem:50 }
        ],
        primaryDate:'3 juin 2025',
        primaries:{
          rep:{ title:'Primaire Républicaine (Top 2)', note:'Californie utilise un système de primaire unique. Valadao s\'est qualifié avec 52%.',
            candidates:[
              { name:'David Valadao', initials:'DV', desc:'Représentant sortant', score:'52%', class:'r', badge:'badge-qualified', badgeText:'Qualifié', isWinner:true },
              { name:'Chris Mathys', initials:'CM', desc:'Homme d\'affaires conservateur', score:'40%', class:'r', badge:'badge-eliminated', badgeText:'Battu', isWinner:false },
              { name:'Autres', initials:'—', desc:'Candidats mineurs', score:'8%', class:'r', badge:'badge-eliminated', badgeText:'Éliminés', isWinner:false }
            ],
            polls:[
              { institute:'Résultat primaire', date:'3 Jun 2025', values:['52%','40%','8%'] },
              { institute:'PPIC', date:'Mai 2025', values:['50%','38%','12%'] }
            ]
          },
          dem:{ title:'Primaire Démocrate (Top 2)', note:'Jasmeet Bains a remporté la primaire avec 60% des voix.',
            candidates:[
              { name:'Jasmeet Bains', initials:'JB', desc:'Médecin · Députée d\'État de Californie', score:'60%', class:'d', badge:'badge-winner', badgeText:'🏆 Vainqueur', isWinner:true },
              { name:'Rudy Salas', initials:'RS', desc:'Ancien candidat 2022', score:'33%', class:'d', badge:'badge-eliminated', badgeText:'Battu', isWinner:false },
              { name:'Autres', initials:'—', desc:'Candidats mineurs', score:'7%', class:'d', badge:'badge-eliminated', badgeText:'Éliminés', isWinner:false }
            ],
            polls:[
              { institute:'Résultat primaire', date:'3 Jun 2025', values:['60%','33%','7%'] },
              { institute:'PPIC', date:'Mai 2025', values:['57%','35%','8%'] }
            ]
          }
        }
      }
    ...
    document.querySelectorAll('.cat-tab').forEach(tab => {
      tab.addEventListener('click', function() {
        ...
        if (firstRace) {
          ...
          document.getElementById('detail-container').innerHTML = '<div class="card"><p style="text-align:center;color:rgba(255,255,255,0.4);padding:2rem;">Sélectionnez une course</p></div>';
        }
      });
    });
      ...
      container.innerHTML = `
        <div class="card">
          ...
          <div class="poll-footer">
            <span>📊 Marge ${data.marginError}</span><span>👥 ${data.sample} · ${data.pollDates}</span>
          </div>
          <div style="margin-top:1.2rem;">
            <h4 style="color:rgba(255,255,255,0.55);font-size:0.8rem;text-transform:uppercase;">Probabilités</h4>
          ...
          <div style="margin-top:1.2rem;">
            <h4 style="color:rgba(255,255,255,0.55);font-size:0.8rem;text-transform:uppercase;">📈 Sondages</h4>
            ...
            <div class="chart-legend">
              ...
              <span style="opacity:0.55;">— Moyenne mobile (3)</span>
            </div>
          </div>
        </div>
        ...
        <div class="card">
          <div class="card-header">
            ...
            <div style="color:rgba(255,255,255,0.5);font-size:0.8rem;">📅 ${data.primaryDate}</div>
          </div>
          ...
          <div class="prim-panel active" id="panel-rep">
            <div class="primary-grid">
              ${data.primaries.rep.candidates.map(c => `
                <div class="candidate-card ${c.isWinner?'winner':''}">
                  ...
                  <div class="cand-name">${c.name}</div><div class="cand-desc">${c.desc}</div>
            ...
            <table class="primary-table">
              <thead><tr><th>Institut</th><th>Date</th>${data.primaries.rep.candidates.map(c => `<th>${c.name.split(' ').pop()}</th>`).join('')}</tr></thead>
              <tbody>${data.primaries.rep.polls.map((p,i) => `<tr class="${i===0?'highlight-row':''}"><td>${p.institute}</td><td>${p.date}</td>${p.values.map(v => `<td class="${data.primaries.rep.candidates[0].class==='r'?'r-col':'d-col'}">${v}</td>`).join('')}</tr>`).join('')}</tbody>
            </table>`:''}
          ...
          <div class="prim-panel" id="panel-dem">
            ...
            <table class="primary-table">
              <thead><tr><th>Institut</th><th>Date</th>${data.primaries.dem.candidates.map(c => `<th>${c.name.split(' ').pop()}</th>`).join('')}</tr></thead>
              <tbody>${data.primaries.dem.polls.map((p,i) => `<tr class="${i===0?'highlight-row':''}"><td>${p.institute}</td><td>${p.date}</td>${p.values.map(v => `<td class="${data.primaries.dem.candidates[0].class==='r'?'r-col':'d-col'}">${v}</td>`).join('')}</tr>`).join('')}</tbody>
            </table>`:''}
    ...
    function drawChart(polls, candidates) {
      ...
      const pad={top:22,right:28,bottom:32,left:38},minY=30,maxY=55;
      let points=[];
      function draw(){
