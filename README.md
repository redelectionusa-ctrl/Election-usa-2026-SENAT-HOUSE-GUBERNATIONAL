...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .main-title { font-size:clamp(2rem,5vw,4rem); font-weight:900; letter-spacing:0.2em; color:#fff; text-transform:uppercase; text-shadow:0 0 20px rgba(255,215,150,0.3); }
    .main-subtitle { font-size:clamp(1rem,2.5vw,1.8rem); color:rgba(255,255,255,0.85); letter-spacing:0.3em; text-transform:uppercase; border-top:1.5px solid rgba(255,215,150,0.5); display:inline-block; padding-top:0.6rem; margin-top:0.5rem; }
    .main-content { width:100%; max-width:1400px; }
    .category-tabs { display:flex; gap:0.5rem; margin-bottom:1.5rem; flex-wrap:wrap; }
    .cat-tab { background:rgba(0,0,0,0.4); border:1px solid rgba(255,255,255,0.2); color:white; padding:1rem 1.5rem; font-size:1rem; font-weight:700; text-transform:uppercase; letter-spacing:0.1em; cursor:pointer; flex:1; min-width:130px; text-align:center; transition:0.2s; }
    .cat-tab:hover { background:rgba(255,255,255,0.1); }
    .cat-tab.active { background:rgba(255,255,255,0.15); border-color:rgba(255,215,150,0.6); box-shadow:0 0 25px rgba(255,215,150,0.2); }
    ...
    .race-item { background:rgba(0,0,0,0.35); border:1px solid rgba(220,200,180,0.18); padding:0.9rem 1.5rem; cursor:pointer; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:0.8rem; border-radius:4px; }
    .race-item:hover { background:rgba(255,255,255,0.08); border-color:rgba(255,215,150,0.4); }
    .race-item.active { background:rgba(255,255,255,0.12); border-color:rgba(255,215,150,0.7); box-shadow:0 0 30px rgba(255,215,150,0.15); }
    .race-info h3 { color:#f0e6d2; font-size:1.05rem; margin-bottom:0.2rem; }
    .race-info p { color:rgba(255,255,255,0.5); font-size:0.75rem; }
    .race-tag { padding:0.2rem 0.8rem; font-weight:700; text-transform:uppercase; font-size:0.65rem; letter-spacing:0.08em; white-space:nowrap; border-radius:3px; }
    .safe-r { background:#4a0000; color:#ff8a80; border:1px solid #b71c1c; }
    ...
    .card { width:100%; background:rgba(0,0,0,0.35); backdrop-filter:blur(10px); border:1px solid rgba(220,200,180,0.18); box-shadow:0 20px 35px -8px rgba(0,0,0,0.6); padding:1.8rem; margin-bottom:1.2rem; border-radius:6px; }
    .card-header { display:flex; align-items:center; justify-content:space-between; margin-bottom:1.8rem; border-bottom:2px solid rgba(255,255,255,0.12); padding-bottom:1.4rem; flex-wrap:wrap; gap:1rem; }
    .card-title { font-size:1.4rem; font-weight:600; letter-spacing:0.1em; text-transform:uppercase; color:#f0e6d2; display:flex; align-items:center; gap:1rem; }
    .state-svg { width:50px; height:50px; filter:drop-shadow(0 4px 8px rgba(0,0,0,0.5)); opacity:0.8; }
    .rep { color:#ff9e8f; } .dem { color:#8fc1ff; }
    .candidate-row { margin-bottom:1.4rem; }
    .candidate-label { display:flex; justify-content:space-between; margin-bottom:0.4rem; flex-wrap:wrap; }
    .party-name { font-size:1.1rem; font-weight:700; letter-spacing:0.06em; text-transform:uppercase; }
    .party-pct { font-size:1.4rem; font-weight:800; }
    .bar-wrap { width:100%; height:36px; background:#121212e0; border:1px solid rgba(255,255,255,0.12); overflow:hidden; border-radius:2px; }
    ...
    .poll-footer { margin-top:1rem; display:flex; justify-content:space-between; color:rgba(255,240,220,0.7); border-top:1px solid rgba(255,255,240,0.08); padding-top:0.8rem; font-size:0.75rem; }
    .gauge-container { margin:1.5rem 0 1rem; }
    .gauge-label { display:flex; justify-content:space-between; font-size:0.6rem; color:rgba(255,255,255,0.4); margin-bottom:0.3rem; text-transform:uppercase; }
    .gauge-bar { height:12px; background:linear-gradient(to right, #3e0000, #6a0a0a, #7a1515, #8a2020, #5a4a00, #0a2f5a, #0a2f6a, #0a2060, #001a4d); border-radius:6px; position:relative; margin-bottom:0.4rem; }
    .gauge-arrow { position:absolute; top:-8px; transform:translateX(-50%); font-size:1.2rem; filter:drop-shadow(0 2px 4px rgba(0,0,0,0.6)); transition:left 0.3s; }
    .gauge-pos-text { text-align:center; font-size:0.75rem; color:rgba(255,255,255,0.7); font-weight:600; }
    .projections-bar { display:flex; height:8px; margin:0.6rem 0; background:#121212; border-radius:4px; overflow:hidden; }
    ...
    .prim-tab { background:rgba(0,0,0,0.4); border:1px solid rgba(255,255,255,0.2); color:white; padding:0.5rem 0.8rem; font-size:0.85rem; font-weight:600; text-transform:uppercase; cursor:pointer; flex:1; text-align:center; border-radius:4px; }
    ...
    .candidate-card { background:rgba(0,0,0,0.3); border:1px solid rgba(255,255,255,0.1); padding:1rem; text-align:center; border-radius:4px; }
    ...
    .chart-container { position:relative; width:100%; height:230px; margin:0.6rem 0; }
    canvas { display:block; width:100%; height:100%; background:rgba(10,10,15,0.3); border:1px solid rgba(255,255,255,0.1); cursor:crosshair; border-radius:4px; }
    .chart-tooltip { position:absolute; background:rgba(20,20,30,0.95); color:white; padding:6px 9px; border:1px solid rgba(255,255,255,0.25); font-size:10px; font-weight:600; pointer-events:none; z-index:100; text-align:center; line-height:1.3; white-space:nowrap; border-radius:4px; }
    ...
    .legend-dot { width:8px; height:8px; display:inline-block; margin-right:3px; border-radius:50%; } .legend-dot.r { background:#e53935; } .legend-dot.d { background:#1e88e5; }
  </style>
</head>
<body>
...
<script>
(function() {
  // ========== FORMES SVG POUR QUELQUES ÉTATS ==========
  const stateShapes = {
  ...
  // ========== POSITION DE LA FLÈCHE SELON LE RATING ==========
  const ratingPositions = {
    'Safe R': 5, 'Likely R': 16, 'Lean R': 27, 'Tilt R': 38,
    'Tossup': 50, 'Tilt D': 62, 'Lean D': 73, 'Likely D': 84, 'Safe D': 95
  };

  // ========== DONNÉES DE BASE ==========
  const racesData = {};

  // Courses House (ajoutées au panier House plus tard)
  ...
  // Courses Sénat (triées Safe R → Safe D)
  const senateRaces = [
  ...
  // ---------- FONCTION POUR CONSTRUIRE UNE ENTRÉE DE COURSE ----------
  function buildRaceEntry(r, type) {
    const initials = (n) => n.split(' ').map(w => w[0]).join('');
    const polls = [];
    if (r.polls) {
      r.polls.forEach(p => polls.push(p));
    } else {
      for (let m = 1; m <= 9; m++) {
    ...
    return {
      ...
      primaries: {
        rep: { note: 'Source : Ballotpedia. Mise à jour après les primaires.', cands: repCands, polls: [] },
        dem: { note: 'Source : Ballotpedia. Mise à jour après les primaires.', cands: demCands, polls: [] }
      }
    };
  }

  // Construire toutes les courses
  senateRaces.forEach(r => { racesData[r.id] = buildRaceEntry(r, 'senate'); });
  houseRacesData.forEach(r => { 
    r.title = r.title || `House · ${r.state || r.id}`;
    racesData[r.id] = buildRaceEntry(r, 'house'); 
  });

  // Remplir les listes
  function populateList(listId, raceIds, type) {
    const list = document.getElementById(listId);
    const items = raceIds.map(id => {
      const r = racesData[id];
      if (!r) return '';
      const name1 = r.candidates[0].name;
      const name2 = r.candidates[1].name;
      return `<div class="race-item" data-race="${id}"><div class="race-info"><h3>${r.stateName || r.title.replace(/^(House|Sénat) · /, '')}</h3><p>${name1} (R) vs ${name2} (D)</p></div><div class="race-tag ${r.ratingClass}">${r.rating}</div></div>`;
    }).join('');
    list.innerHTML = items;
  }

  const senateIds = senateRaces.map(r => r.id);
  const houseIds = houseRacesData.map(r => r.id);
  populateList('senate-races', senateIds, 'senate');
  populateList('house-races', houseIds, 'house');
  ...
  function showRaceDetail(raceId) {
    ...
    const stateName = data.stateName || data.title.split('·')[1]?.trim() || '';
    const shapeSvg = stateShapes[stateName] || '';
    const arrowPos = ratingPositions[data.rating] || 50;
    const totalP = (data.proj.rSafe||0)+(data.proj.rLikely||0)+(data.proj.rLean||0)+(data.proj.rTilt||0)+(data.proj.tossup||0)+(data.proj.dTilt||0)+(data.proj.dLean||0)+(data.proj.dLikely||0)+(data.proj.dSafe||0) || 100;
    const scale = 100/totalP;

    container.innerHTML = `
      <div class="card">
        <div class="card-header">
          <div class="card-title">
            ${shapeSvg ? `<div class="state-svg">${shapeSvg}</div>` : ''}
            ${data.title}
          </div>
          <div class="race-tag ${data.ratingClass}">${data.rating}</div>
        </div>
        ...
        <div class="gauge-container">
          <div class="gauge-label" style="display:flex; justify-content:space-between;">
            ${['Safe R','Likely R','Lean R','Tilt R','Tossup','Tilt D','Lean D','Likely D','Safe D'].map(s => `<span>${s}</span>`).join('')}
          </div>
          <div class="gauge-bar">
            <div class="gauge-arrow" style="left:${arrowPos}%;">▼</div>
          </div>
          <div class="gauge-pos-text">Position actuelle : <strong style="color:${data.ratingClass.includes('r')?'#ff9e8f':data.ratingClass.includes('d')?'#8fc1ff':'#ffe082'}">${data.rating}</strong></div>
        </div>

        <div style="margin-top:1rem;"><h4 style="color:rgba(255,255,255,0.5);font-size:0.7rem;text-transform:uppercase;">Projections détaillées</h4>
          <div class="projections-bar">
            <div class="p-safe-r" style="width:${(data.proj.rSafe||0)*scale}%"></div>
            <div class="p-likely-r" style="width:${(data.proj.rLikely||0)*scale}%"></div>
            <div class="p-lean-r" style="width:${(data.proj.rLean||0)*scale}%"></div>
            <div class="p-tilt-r" style="width:${(data.proj.rTilt||0)*scale}%"></div>
            <div class="p-tossup" style="width:${(data.proj.tossup||0)*scale}%"></div>
            <div class="p-tilt-d" style="width:${(data.proj.dTilt||0)*scale}%"></div>
            <div class="p-lean-d" style="width:${(data.proj.dLean||0)*scale}%"></div>
            <div class="p-likely-d" style="width:${(data.proj.dLikely||0)*scale}%"></div>
            <div class="p-safe-d" style="width:${(data.proj.dSafe||0)*scale}%"></div>
          </div>
          <div style="font-size:0.6rem;color:rgba(255,255,255,0.35);display:flex;justify-content:space-between;">${['Safe R','Likely R','Lean R','Tilt R','Tossup','Tilt D','Lean D','Likely D','Safe D'].map(s=>'<span>'+s+'</span>').join('')}</div>
        </div>
        <div style="margin-top:1rem;color:rgba(255,255,255,0.5);font-size:0.75rem;text-align:center;">${data.seats}</div>

        <div style="margin-top:1.2rem;"><h4 style="color:rgba(255,255,255,0.5);font-size:0.7rem;text-transform:uppercase;">📈 Sondages</h4>
  ...
  function drawChart(polls, candidates) {
    ...
    function draw() {
      ...
      polls.forEach((p, i) => { ctx.fillStyle = 'rgba(255,255,255,0.45)'; ctx.font = '9px Segoe UI'; ctx.textAlign = 'center'; ctx.fillText(p.date, gX(i), pad.top + cH + 10); });
  ...
  document.querySelectorAll('.cat-tab').forEach(tab => {
    tab.addEventListener('click', function() {
      ...
      const list = this.dataset.cat === 'house' ? document.querySelector('#house-races .race-item') : document.querySelector('#senate-races .race-item');
      if (list) list.click();
    });
  });
  ...
  // Afficher la première course du Sénat par défaut
  const firstSenateItem = document.querySelector('#senate-races .race-item');
  if (firstSenateItem) firstSenateItem.click();
  else {
    const firstHouseItem = document.querySelector('#house-races .race-item');
    if (firstHouseItem) firstHouseItem.click();
  }
})();
</script>
